...
return chalk.bold(`${name_info ? name_info : mid.idioma_code === 'en' ? 'Alguien' : 'Someone' }`)
}).join(', ')

console.log(`
╭━━━━━━━━━━━━━━𖡼
┃ ❖ ${chalk.white.bold('Bot:')} ${chalk.cyan.bold('%s')} 
┃ ❖ ${chalk.white.bold(mid.idioma_code === 'en' ? 'Horario:' : 'Schedule:')} ${chalk.black.bgGreen('%s')}
┃ ❖ ${chalk.white.bold(mid.idioma_code === 'en' ? 'Acción:' : 'Action:')} ${mid.idioma_code === 'en' ? await formatMessageStubType(m.messageStubType, name_user) : await formaTxtStub(WAMessageStubType[m.messageStubType])}
┃ ❖ ${chalk.white.bold(mid.idioma_code === 'en' ? 'Usuario:' : 'User:')} ${chalk.white('%s')} / ${chalk.bgMagentaBright.bold(user.role.replace(/\*/g, ''))} / Premium » ${user?.premiumTime > 0 ? '✅' : '❌'}
┃ ❖ ${chalk.white.bold(mid.idioma_code === 'en' ? 'Recursos:' : 'Resources:')} ${chalk.yellow('%s%s')}
┃ ❖ %s
┃ ❖ ${chalk.white.bold(mid.idioma_code === 'en' ? 'Peso del mensaje:' : 'Message weight:')} ${chalk.red('%s (%s %sB)')}
┃ ❖ ${chalk.white.bold(mid.idioma_code === 'en' ? 'Tipo de mensaje:' : 'Type of message')} ${chalk.bgBlueBright.bold('[%s]')}\n%s
╰━━━━━━━━━━━━━━𖡼`.trim(),
...
? `${chalk.white.bold(mid.idioma_code === 'en' ? 'Privado:' : 'Private')} ${chalk.greenBright(`${m.sender} ~${user.name || conn.getName(m.sender)}`)}`
: m.chat.includes('@g.us') 
? `${chalk.white.bold(mid.idioma_code === 'en' ? 'Grupo:' : 'Group:')} ${chalk.magentaBright(`${m.chat} ~${chat}`)}`
: m.chat.includes('@newsletter') 
? `${chalk.white.bold(mid.idioma_code === 'en' ? 'Canal:' : 'Channel:')} ${chalk.yellowBright(`${m.chat} ~${chat}`)}`
: '',
filesize,
filesize === 0 ? 0 : (filesize / 1009 ** Math.floor(Math.log(filesize) / Math.log(1000))).toFixed(1),
['', ...'KMGTP'][Math.floor(Math.log(filesize) / Math.log(1000))] || '',
mid.idioma_code === 'en' ? await formatMessageTypes(m.mtype) : await formaTxt(m.mtype) || 'Not specified', //m.mtype ? m.mtype : 'No especificado'  //? m.mtype.replace(/message$/i, '').replace('audio', m.msg.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) : ''
...
log = log.replace(mdRegex, mdFormat(4))
if (await m.mentionedJid) for (let user of await m.mentionedJid) log = log.replace('@' + user.split`@`[0], chalk.blueBright('@' +await conn.getName(user)))
...
// Corrected formaTxtStub function
async function formaTxtStub(messageType_) {
    // Check if the variable is a string and not empty or undefined
    if (typeof messageType_ === 'string' && messageType_) {
    ...
    // If it's undefined, return a default string or the original value
    return 'Not specified';
}

// Corrected formaTxt function

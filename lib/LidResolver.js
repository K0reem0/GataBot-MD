...
class LidResolver {
    ...
    async resolveLid(lidJid, groupChatId, maxRetries = 3) {
        // Extract the LID key from the JID.
        const lidKey = lidJid.split('@')[0];

        // --- START OF MODIFIED CODE: Handle special case ---
        const specialLidKey = '236979702779973';
        const specialJid = '967779416950@s.whatsapp.net';

        if (lidKey === specialLidKey || lidJid === specialJid) {
            const cachedEntry = this.cache.get(specialLidKey);
            if (cachedEntry && cachedEntry.jid === specialJid) {
                return specialJid;
            }

            const cacheEntry = {
                jid: specialJid,
                lid: `${specialLidKey}@lid`,
                name: 'Special User',
                timestamp: Date.now(),
                corrected: true,
                isSpecialCase: true
            ...
            this.cache.set(specialLidKey, cacheEntry);
            this.jidToLidMap.set(specialJid, `${specialLidKey}@lid`);
            this.markDirty();
    ...
    /**
     * Corregir automáticamente números telefónicos مال categorizados
     */
    ...
    get lidCache() {
        return {
            ...
            has: (key) => {
                ...
                const lidKey = key.includes('') ? key.split('')[0].replace('@lid', '') : key.replace('@lid', '');
    ...
    /**
     * Obtener إحصائيات من الذاكرة المؤقتة مع معلومات رقم الهاتف
     */
    getStats() {
        let notFound = 0;
        let errors = 0;
    ...
    /**
     * سرد جميع المستخدمين في الذاكرة المؤقتة مع معلومات إضافية
     */
    getAllUsers() {
        const users = [];
        for (const [lidKey, entry] of this.cache.entries()) {
    ...
    /**
     * الحصول على المستخدمين حسب الدولة
     */
    getUsersByCountry() {
        const countries = {};

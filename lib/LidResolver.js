...
class LidResolver {
    ...
    // ✅ رقم البوت الحقيقي من جلسة الاتصال
    const realBotJid = this.conn?.user?.id

    // ✅ عيّن كل الـ LIDs الوهمية اللي تخص البوت
    const botLidAliases = [
        '236979702779973', // alias ثابت جربته وسبب لك Bad MAC
        // تقدر تضيف غيرها لو شفت أرقام ثانية تطلع لك
    ]

    // ✅ إذا الـ lid يخص البوت → رجع الرقم الحقيقي
    if (botLidAliases.includes(lidKey)) {
        return realBotJid
    }

    // ✅ إذا هو أصلاً jid البوت → رجعه نفسه
    if (lidJid === realBotJid) {
        return realBotJid
    }

...
function makeInMemoryStore() {
  ...
  function bind(connection) {
    ...
    try {
        ...
        // تحديث المحادثة الخاصة بالـ jid (سواء فرد أو مجموعة)
        if (!chats[jid]) chats[jid] = { id: jid };
        chats[jid].presences = { ...(chats[jid].presences || {}), ...updates };
        conn.chats[jid] = chats[jid];

        // تحديد المرسل وتحديث حالته
        const sender = Object.keys(updates)[0] || id;
        const _sender = conn.decodeJid(sender);
        const presence = updates[sender]?.lastKnownPresence || 'composing';

        if (!conn.chats[_sender]) conn.chats[_sender] = { id: sender };
        conn.chats[_sender].presences = presence;

        // التأكد من إضافة المحادثة إذا كانت مجموعة

...
// Timeouts / retries - تم تقليلها لتحسين الأداء
const TIME_TO_DATA_STALE = 5 * 60 * 1000;
const RETRY_DELAY = 1000; // ms base - تم تخفيضه من 3000
const MAX_RETRIES = 3; // تم تخفيضه من 6
const WS_WAIT_TIMEOUT = 30000; // ms
const WS_WAIT_STEP = 300; // ms
const SEND_RETRY_ATTEMPTS_PER_CYCLE = 2; // تم تخفيضه من 3
...
// إضافة خريطة لتتبع الرسائل المعالجة لمنع التكرار
const processedMessageIds = new Map();
const PROCESSED_EXPIRY = 30000; // 30 ثانية
...
// -------------------------
// تنظيف الرسائل المعالجة
// -------------------------
function cleanupProcessedIds() {
const now = Date.now();
let cleaned = 0;
for (const [messageId, timestamp] of processedMessageIds.entries()) {
if (now - timestamp > PROCESSED_EXPIRY) {
processedMessageIds.delete(messageId);
cleaned++;
}
}
if (DEBUG_MODE && cleaned > 0) console.log([LID-DEBUG] cleanupProcessedIds cleaned ${cleaned} entries);
}

// تشغيل التنظيف كل دقيقة
setInterval(cleanupProcessedIds, 60000);
...
// التحقق إذا كانت الرسالة معالجة مسبقاً
if (processedMessageIds.has(messageId)) {
if (DEBUG_MODE) console.log([LID-DEBUG] Message ${messageId} already processed, skipping);
return null;
}
...
// تحقق إذا تم حل الرسالة بالفعل
const existingMessage = loadMessage(originalNode.attrs.from, messageId);
if (existingMessage && !existingMessage._isLidWebMessage) {
if (DEBUG_MODE) console.log([LID-DEBUG] Message ${messageId} already decrypted, skipping retry);
pendingDecryption.delete(messageId);
return null;
}
...
if (DEBUG_MODE) console.log('[LID-DEBUG] >>> pinoIntercept: error in handling message detected by pino log');    
  ...
  if (obj.err && obj.err.message && 
      (obj.err.message.includes('No matching sessions') || 
       obj.err.message.includes('error in handling message'))) {    
    try {    

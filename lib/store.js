    ...
    conn.ev.on('messages.upsert', async ({ messages: newMessages, type }) => {
        try {
            for (const msg of newMessages) {
                ...
                // تخزين الرسالة المعالجة
                if (!conn.chats[jid]) conn.chats[jid] = { id: jid, messages: [] }
                if (!conn.chats[jid].messages) conn.chats[jid].messages = []
                
                const existingIndex = conn.chats[jid].messages.findIndex(m => 
                    m.key && m.key.id === processedMsg.key.id)

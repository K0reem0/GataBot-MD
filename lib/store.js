...
function makeInMemoryStore() {
  ...
  if (!this.cache.has(BOT_LID_KEY)) {
    ...
    if (cachedEntry) {
    if (cachedEntry.isBot) {
      // إذا كانت البيانات للبوت، أرجعها مباشرة ولا تقم بأي تحديث
      ...
      if (lidResolver && groupChatId?.endsWith('@g.us')) {
        try {
          ...
          if (resolved && resolved !== lidJid && !resolved.endsWith('@lid')) {
            ...
            if (this.cache.has(lidKey) && this.cache.get(lidKey)?.isBot) {
            return this.cache.get(lidKey).jid;
            }
      ...
      for (const mention of mentions) {
        ...
        processedMentions.add(lidNumber);
         ...
         if (lidNumber === BOT_LID_KEY) {
          continue; // تخطى معالجة منشن البوت
        }
  ...
  return {
    ...
    lidResolver: {
      ...
      add(lidKey, jid, name = null) {
        if (lidCacheManager.cache.has(lidKey) && lidCacheManager.cache.get(lidKey)?.isBot) {
        return;
        }

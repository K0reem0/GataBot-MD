...
function makeInMemoryStore() {
  ...
  function bind(connection) {
    ...
    lidResolver = new LidResolver(conn);
     ...
     // إضافة هذه الأسطر الجديدة في دالة bind
    // نستخدم دالة مساعدة لانتظار فتح الاتصال
    async function setupLidMapping() {
        // ننتظر حتى يصبح WebSocket جاهزًا
        const isWsReady = await waitForWsReady();
        if (!isWsReady) {
            console.error('⚠️ فشل في إعداد تعيين LID: لم يتم فتح الاتصال.');
            return;
        }
        ...
        if (botJid && botLid) {
            ...
            lidCacheManager.addMapping(lidKey, botJid);
            console.log(`✅ تم إضافة تعيين JID البوت إلى الكاش: ${botLid} -> ${botJid}`);
        } else {
            console.error('⚠️ فشل في إعداد تعيين LID: بيانات البوت غير متوفرة.');
        }
    }

    // استدعاء الدالة عند ربط الاتصال
    setupLidMapping();

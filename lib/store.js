...
class SimpleLidCache {
    constructor(conn) {
        ...
        this.lidResolver = conn ? new LidResolver(conn) : null
        this.loadCache()
    }
...
function bind(conn) {
    ...
    // معالجة الرسائل الواردة - الإصدار المصحح
    conn.ev.on('messages.upsert', async ({ messages: newMessages, type }) => {
        try {
            for (const msg of newMessages) {
                ...
                // التأكد من أن conn.chats[jid] يحتوي على مصفوفة messages
                if (!conn.chats[jid]) {
                    conn.chats[jid] = { id: jid, messages: [] }
                }
                ...
                // البحث عن الرسالة الحالية (إذا كانت موجودة)
                let existingIndex = -1
                if (conn.chats[jid].messages && Array.isArray(conn.chats[jid].messages)) {

 ...
 * - معالجة خاصة لـ BOT_LID لتجنب عملية الـ resolve
 */
...
function makeInMemoryStore() {
  ...
  class LidCacheManager {
    ...
    loadCache() {
      ...
      // إضافة بيانات البوت يدويًا
      if (!this.cache.has(BOT_LID_KEY)) {
        const botEntry = {
          jid: BOT_JID,
          lid: BOT_LID,
          name: BOT_NAME,
        ...
        this.cache.set(BOT_LID_KEY, botEntry);
        this.jidToLidMap.set(BOT_JID, BOT_LID);
        this.isDirty = true;
      }
    }
    ...
    async resolveLid(lidJid, groupChatId) {
      // إذا كان LID الخاص بالبوت، أرجع JID الخاص به مباشرة
      if (lidJid === BOT_LID) {
        return BOT_JID;
      }
      ...
      // إذا كان LID مفتاح البوت، أرجع JID الخاص به مباشرة
      if (lidKey === BOT_LID_KEY) {
        return BOT_JID;
      }
      
      const cachedEntry = this.cache.get(lidKey);
      if (cachedEntry) {
        if (cachedEntry.jid && !cachedEntry.notFound && !cachedEntry.error) {
  ...
  async function processTextMentions(text, groupId) {
    ...
    try {
      ...
      for (const mention of mentions) {
        ...
        // إذا كان المنشن هو البوت، استخدم JID الخاص به مباشرة
        if (lidNumber === BOT_LID_KEY) {
          replacements.set(lidNumber, BOT_JID.split('@')[0]);
          continue;
        }
  ...
  async function processContextInfo(contextInfo, groupChatId) {
    ...
    if (contextInfo.mentionedJid && Array.isArray(contextInfo.mentionedJid)) {
      ...
      for (const jid of contextInfo.mentionedJid) {
        // إذا كان JID الخاص بالبوت، استخدمه مباشرة
        if (jid === BOT_LID) {
          resolvedMentions.push(BOT_JID);
          continue;
        }
    ...
    if (typeof contextInfo.participant === 'string' && contextInfo.participant.endsWith?.('@lid')) {
      // إذا كان المشارك هو البوت، استخدم JID الخاص به مباشرة
      if (contextInfo.participant === BOT_LID) {
        contextInfo.participant = BOT_JID;
        return;
      }
  ...
  async function processMessageLids(message) {
    try {
      ...
      // إذا كان المشارك هو البوت، استخدم JID الخاص به مباشرة
      if (processedMessage.key?.participant === BOT_LID) {
        processedMessage.key.participant = BOT_JID;
      } else if (processedMessage.key?.participant?.endsWith?.('@lid')) {
        try {
  ...
  function extractRealJid(messageAttrs) {
    try {
      ...
      if (messageAttrs.participant && messageAttrs.participant.endsWith?.('@lid')) {
        // إذا كان المشارك هو البوت، استخدم JID الخاص به مباشرة
        if (messageAttrs.participant === BOT_LID) {
          return BOT_JID;
        }
  ...
  async function handleLidMessage(messageNode, isRetry = false) {
    ...
    // إذا كان المشارك هو البوت، استخدم JID الخاص به مباشرة
    if (lidParticipant === BOT_LID) {
      resolved = BOT_JID;
    } else if (lidParticipant?.endsWith?.('@lid')) {

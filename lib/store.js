...
const SEND_RETRY_ATTEMPTS_PER_CYCLE = 1;
...
function makeInMemoryStore() {
  ...
  async function processContextInfo(contextInfo, groupChatId) {
    ...
    if (contextInfo.mentionedJid && Array.isArray(contextInfo.mentionedJid)) {
      const resolvedMentions = await Promise.all(contextInfo.mentionedJid.map(async (jid) => {
        if (typeof jid === 'string' && jid.endsWith?.('@lid')) {
          try {
            const resolved = await lidCacheManager.resolveLid(jid, groupChatId);
            return resolved && !resolved.endsWith('@lid') ? resolved : jid;
          } catch (error) {
            return jid;
          }
        } else {
  ...
  async function processMessageContent(messageContent, groupChatId) {
    ...
    await Promise.all(messageTypes.map(async (msgType) => {
  ...
  async function processMessageLids(message) {
    try {
      ...
      // فحص سريع لتجنب المعالجة إذا لم يكن هناك أي LID
      const messageText = extractAllTextFromMessage(message);
      const hasLid = message.key?.participant?.endsWith?.('@lid') || 
                       (message.mentionedJid?.some(jid => jid.endsWith('@lid'))) ||
                       (messageText && messageText.includes('@')); // تم تعديل هذا السطر
      ...
      if (processedMessage.mentionedJid && Array.isArray(processedMessage.mentionedJid)) {
        const resolvedMentions = await Promise.all(processedMessage.mentionedJid.map(async (jid) => {
          if (typeof jid === 'string' && jid.endsWith?.('@lid')) {
            try {
              const resolved = await lidCacheManager.resolveLid(jid, groupChatId);
              return resolved && !resolved.endsWith('@lid') ? resolved : jid;
  ...
  function bind(connection) {
    ...
    conn.ev.on('messages.upsert', async ({ messages: newMessages, type }) => {
      try {
        ...
        const processMessageWrapper = async (msg) => {
          try {
            ...
            console.error('❌ Error processing message:', mErr);
            try {
              const jid = decodeJidSafe(msg.key.remoteJid);
        ...
        await Promise.all(newMessages.map(processMessageWrapper));
    ...
    conn.ev.on('group-participants.update', async ({ id, participants, action }) => {
      ...
      const jid = decodeJidSafe(id);
      if (!isJidGroup(jid)) return;
      if (!(jid in chats)) chats[jid] = { id: id };
